...
def get_mask_img(img, patch_size):
    ...
    patch_rows = rows // 8  # 128 / 8 = 16
    patch_cols = cols // 8  # 128 / 8 = 16

    # 创建patch索引
    # patch_indices = [(i // 12, i % 12) for i in range(16 * 12)]
    patch_indices = [(i // 16, i % 16) for i in range(16 * 16)]

    # 随机选择160个patches
    selected_patches = random.sample(patch_indices, 200)
    ...
    # )
    masked_img1 = mask_patches_on_image(img, selected_patches[:100], patch_size, color=black)
    masked_img2 = mask_patches_on_image(img, selected_patches[100:], patch_size, color=white)
...
if __name__ == "__main__":
    ...
    output_pathA = "/data2/wait/bisheCode/DDPM_Fusion/dataset/TXCJ128/val/ImageA"
    output_pathB = "/data2/wait/bisheCode/DDPM_Fusion/dataset/TXCJ128/val/ImageB"
    # 确保输出文件夹存在，如果不存在则创建
