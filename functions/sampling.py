...
def normalize_feature_maps(feature_maps):
    ...
    min_values, _ = torch.min(
        feature_maps[:, :, int(0.1 * H) : int(0.9 * H), int(0.1 * W) : int(0.9 * W)]
        .contiguous()
        .view(feature_maps.shape[0], feature_maps.shape[1], -1),
        dim=2,
    )
...
def generalized_steps_multi(x, x_conds, seq, models, b=0.0, eta=1.0):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        x0_preds = []
        ...
        for counter, (i, j) in enumerate(zip(reversed(seq), reversed(seq_next))):
            ...
            if counter < 1:
                ...
                weight_A = torch.sigmoid(10 * (x0_list[0] - x0_list[1] - 0.0))
                weight_B = (
                    1 - weight_A
                )  # torch.abs(x0_list[1]) / (torch.abs(x0_list[1]) + torch.abs(x0_list[0]) + 1e-6)
...
def generalized_steps_multi_weight(
    x, x_conds, seq, models, model_weight=None, b=0.0, eta=1.0
):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        ...
        for counter, (i, j) in enumerate(zip(reversed(seq), reversed(seq_next))):
            ...
            if model_weight is not None:
                if (
                    counter == 0
                ):  # If it is noise from the first round of estimation, fused using preset weights [0.5, 0,5]
                    weight_A = (
                        0.5 * torch.ones_like(x_conds[1], device=x.device)[:, :1, ::]
                    ...
                    if (
                        counter == 1
                    ):  ## The main consideration is that the noise estimated in the first step is not accurate enough
                        x0_ir = x0_list[0]
                        # weight_input = torch.cat([x0_ir, x0_ir, tried_weight], dim=1)
                        weight_input = torch.cat(
                            [x0_ir, x0_list[0], weights[-1]], dim=1
                        )
                        ...
                        weight_input = torch.cat(
                            [x0_ir, last_x0_ir, weights[-1]], dim=1
                        )  ##
...
def generalized_steps_multi_weight_train(
    x, x_conds, seq, models, model_weight=None, b=0.0, eta=1.0
):
    n = x.size(0)
    seq_next = [-1] + list(seq[:-1])
    ...
    for counter, (i, j) in enumerate(zip(reversed(seq), reversed(seq_next))):
        ...
        if model_weight is not None:
            if (
                counter == 0
            ):  # If the noise is from the first round of estimation, it is fused using the preset weights [0.5, 0,5]
                weight_A = (
                    0.5 * torch.ones_like(x_conds[1], device=x.device)[:, :1, ::]
                )  ## channel维度是1
                weight_B = 1 - weight_A
...
def generalized_steps_mif(x, x_conds, seq, models, model_weight=None, b=0.0, eta=1.0):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        x0_preds = []
        ...
        for counter, (i, j) in enumerate(zip(reversed(seq), reversed(seq_next))):
            ...
            if model_weight is not None:
                if counter == 0:
                    weight_A = (
                        -1.0 * torch.ones_like(x_conds[1], device=x.device)[:, :1, ::]
                    )
                    weight_B = 1 - weight_A
                    weights.append(weight_A)
                    weight_input = torch.cat(
                        [x0_list[0], x0_list[0], weights[-1]], dim=1
                    )
...
def generalized_steps_mif_train(
    x, x_conds, seq, models, model_weight=None, b=0.0, eta=1.0
):
    n = x.size(0)
    seq_next = [-1] + list(seq[:-1])
