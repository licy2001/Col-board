...
class DDPM(object):
    def __init__(self, args, config):
        ...
        # mu_theta_t 的系数
        self.p_mean_coef1 = (
            self.sqrt_alphas_bar * (1 - self.alphas_bar_prev) / (1 - self.alphas_bar)
        )
        self.p_mean_coef2 = (
            torch.sqrt(self.alphas_bar_prev) * self.betas / (1 - self.alphas_bar)
        )
    ...
    def setup(self, args, config):
        self.root_dir = os.path.join(
            config.path.root, "results", "{}".format(config.data.dataset)
        )
        ...
        self.generate_process_dir = os.path.join(
            self.root_dir, config.path.train_process
        )
    ...
    def train(self, DATASET):
        ...
        try:
            for epoch in range(self.start_epoch, self.config.training.n_epochs):
                ...
                for i, (x, y) in enumerate(tqdm(train_loader)):
                    ...
                    loss, x_t, eps, pred_noise, pred_x0 = self.get_loss(
                        x0=x0, x_cond=x_cond
                    )
                    self.logger.info(
                    ...
                    if self.step % 50 == 0:
                        mult_img_dict = {
                            ...
                            "pred_noise": inverse_data_transform(
                                pred_noise[0, ::].detach()
                            ),
                        ...
                        mult_img_dict_path = os.path.join(
                ...
                if epoch % 30 == 0:
                    ...
                    self.logger_val.info(
                        "<epoch:{}\titer:{}>\tpsnr: {:.4f}\tssim: {:.4f}".format(
                            epoch, self.step, avg_psnr, avg_ssim
                        )
                    )
                    if avg_psnr > best_psnr:
                        best_psnr = avg_psnr
                        ckpt_save_path = os.path.join(
                            self.checkpoint_dir, self.args.name + "_" + "best_psnr"
                        )
                        self.logger.info(
                ...
                if epoch % 100 == 0:
                    ckpt_save_path = os.path.join(
                        self.checkpoint_dir, self.args.name + "_epoch_" + str(epoch)
                    )
                    self.logger.info(
                        "Saving models and training states in {}.".format(
                            ckpt_save_path
                        )
                    )
                    save_checkpoint(
        ...
        # except Exception as e:
            ...
            # 保存当前epoch时刻的模型
            ckpt_save_path = os.path.join(
                self.checkpoint_dir, self.args.name + "_interrupt_epoch_" + str(epoch)
            )
            self.logger.info(
    ...
    def sample_backward_step(
        ...
        if clip_x0:
            ...
            mean = (
                self.get_value_t(self.p_mean_coef1, t) * xt
                + self.get_value_t(self.p_mean_coef2, t) * x_0
            )
        else:
            mu_coef1 = 1 / self.get_value_t(self.sqrt_alphas, t)
            mu_coef2 = (1 - self.get_value_t(self.alphas, t)) / self.get_value_t(
    ...
    def get_sample_step(self):
        ...
        time_step = reversed(
            torch.cat((torch.tensor([0], dtype=torch.long), time_step))
        )
    ...
    def ddim_sample(self, xt, x_cond, eta=1.0):
        ...
        for i, p_i in tqdm(seq):
            ...
            c1 = eta * torch.sqrt(
                (1 - alpha_t / alpha_prev) * (1 - alpha_prev) / (1 - alpha_t)
            )
    ...
    def visualize_forward(self, val_loader):
        ...
        cv2.imwrite("/data2/wait/bisheCode/Fusion/diffusion_forward.jpg", res)
    ...
    def val_sample(self, val_loader, epoch):
        ...
        for i, (x, y) in enumerate(tqdm(val_loader)):
            ...
            mult_img_list_path1 = os.path.join(
                os.path.join(image_folder, f"xs{0 + i * 10}.jpg")
            )
            mult_img_list_path2 = os.path.join(
                os.path.join(image_folder, f"x0_preds{0 + i * 10}.jpg")
            )
    ...
    def test_sample(self, DATASET, type):
        ...
        self.logger_val.info(
            "Average PSNR: {:04f}, Average SSIM: {:04f}".format(avg_psnr, avg_ssim)
        )
    ...
    def Fusion_sample(self, fusion_loader, type):
        ...
        for _, (x, y) in enumerate(tqdm(fusion_loader)):
            ...
            # 第二个办法
            # pred_x = self.sample_backward(xt, x_cond, simple_var=True, clip_x0=True)
            xs, x0_preds = self.ddim_sample(xt, x_cond, eta=0.9)
            pred_x = xs[-1]
            pred_x = inverse_data_transform(pred_x)
            x_cond = inverse_data_transform(x_cond)
            ...
            for i in range(n):
                ...
                save_image(
                    x_cond[:, :3, :, :][i], os.path.join(image_folder, "ir", y[i])
                )
                os.makedirs(os.path.join(image_folder, "vi"), exist_ok=True)
                save_image(
                    x_cond[:, 3:, :, :][i], os.path.join(image_folder, "vi", y[i])
                )

    @torch.no_grad()
    def sample_image(self, x_cond, xt, last=True):
        skip = self.config.diffusion.num_diffusion_timesteps // self.args.timesteps
        seq = range(0, self.config.diffusion.num_diffusion_timesteps, skip)
        xs = generalized_steps(xt, x_cond, seq, self.model, self.betas, eta=0.0)
        if last:
            xs = xs[0][-1]
        return xs
    ...
    # 采样融合结果
    @torch.no_grad()
    def Fusion_sample2(self, fusion_loader, type):
        """
        type: 路径名
        """
        self.load_ddm_ckpt(self.args.resume, ema=False, train=False)
...
def generalized_steps(x, x_cond, seq, model, b, eta=1.0):
    ...
    for i, j in zip(reversed(seq), reversed(seq_next)):
        ...
        et = model(torch.cat([x_cond, xt], dim=1), t.float())
